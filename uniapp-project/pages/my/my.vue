  ...
  methods: {
    ...
    startImport: function() {
      ...
      if (sys.platform === 'h5') {
        ...
        self.handleAppImport()
      }
    },
    handleAppImport: function() {
      var self = this
      uni.showLoading({ title: '请选择文件...' })
      goodsExportImport.importFromApp().then(function(res) {
        uni.hideLoading()
        if (res.code === 200) {
          uni.showModal({
            title: '导入成功',
    ...
    downloadTemplate: function() {
      ...
      goodsExportImport.downloadTemplate().then(function(res) {
        ...
        if (res.code === 200) {
          var sys = uni.getSystemInfoSync()
          var msg = '模板下载成功'
          if (sys.platform !== 'h5' && res.data && res.data.path) {
            msg = '模板已保存到: ' + res.data.path
          }
          uni.showToast({
            title: msg,
            icon: 'none',
    ...
    handleExport: function() {
      ...
      goodsExportImport.exportToExcel().then(function(res) {
        ...
        if (res.code === 200) {
          var sys = uni.getSystemInfoSync()
          var msg = '共导出 ' + res.data.length + ' 条商品数据'
          if (sys.platform !== 'h5' && res.data && res.data.path) {
            msg = '共导出 ' + res.data.length + ' 条，文件已保存到: ' + res.data.path
          }
          uni.showModal({
            title: '导出成功',
            content: msg,
